...
class FaceDetector:
    ...
    def rect_to_tuple(self, rect):
        ...
        """
        return rect.top(), rect.right(), rect.width(), rect.height()
    ...
    def detect_faces(self, frame):
        try:
            # attempt to convert the frame to greyscale unless already in that format.
    ...
    def adjust_gamma(self, frame, gamma=1.0):
        # gamma is an optional parameter - if not provided, default value = 1.0
    ...
    def auto_correct_brightness(self, frame, face_roi=None):
        ...
        if face_roi is not None:
            ...
            avg_brightness = np.mean(frame)
            #print(f"original avg brightness {avg_brightness}")  # for testing
        ...
        if avg_brightness > 160:
            ...
            gamma = 1.9  # Decrease gamma for dark images to make them brighter
